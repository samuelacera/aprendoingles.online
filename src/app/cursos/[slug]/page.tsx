      ...
      <section className="relative overflow-hidden border-b border-line">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="inline-flex items-center gap-2 border border-line text-foreground/80 text-sm px-3 py-1 rounded-full">
          ...
          <p className="text-lg text-foreground/70 max-w-2xl">{course.description}</p>
          <div className="flex flex-wrap gap-2 mt-6">
            {course.tags.map((tag) => (
              <span key={tag} className="border border-line text-foreground/60 text-sm px-3 py-1 rounded-full">
                {tag}
              </span>
            ))}
          </div>
          {author && (
            <div className="mt-8 flex flex-col sm:flex-row sm:items-center gap-4 pt-6 border-t border-line">
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={m.title} className="bg-surface border border-line rounded-xl p-5 transition-all duration-300 hover:-translate-y-1 hover:border-gold/30 hover:shadow-lg hover:shadow-gold/5">
                      <p className="font-semibold text-foreground mb-1">{m.title}</p>
                      <p className="text-sm text-foreground/50">{m.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-6">
              {(() => {
                ...
                return sectionEntries.map(([sectionName, sectionLessons]) => (
                  <div key={sectionName} className="bg-surface-2 border border-line rounded-xl overflow-hidden">
                    <div className="flex items-center gap-4 p-5 bg-overlay">
                      ...
                      <span className="text-sm text-foreground/30 ml-auto">{sectionLessons.length} lecciones</span>
                    </div>
                    <div className="border-t border-line px-5 pb-4">
                      {sectionLessons.map((lesson) => {
                        ...
                        return (
                          ...
                          >
                            <span className="text-foreground/20 w-6 text-right shrink-0">{lessonCounter}</span>
                            <span className="text-foreground/70 group-hover:text-gold flex-1 transition-colors">{lesson.h1}</span>
                            <span className="text-xs text-foreground/30">{lesson.duration}</span>
                            {lesson.free ? (
                              <span className="text-xs bg-gold/15 text-gold px-2 py-0.5 rounded-full font-medium">Gratis</span>
                            ) : (
                              <svg className="w-4 h-4 text-foreground/20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              {related.map((r) => (
                <Link
                  key={r.slug}
                  href={`/cursos/${r.slug}`}
                  className={`group bg-surface rounded-xl border border-line p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${accent.hoverBorder} ${accent.hoverShadow}`}
                >
